...
<div class="card-product">
    ...
    <div class="card-product__stock" style="z-index: 20">
        ...
        <? if ($item['LABEL']) { ?>
        <div class="card-product__stock_text active<?/*=$labelPositionClass*/?>" id="<?=$itemIds['STICKER_ID']?>">
            <?
            if (!empty($item['LABEL_ARRAY_VALUE'])) {
                foreach ($item['LABEL_ARRAY_VALUE'] as $code => $value) { ?>
            <span<?=(!isset($item['LABEL_PROP_MOBILE'][$code]) ? ' class="d-none d-sm-block"' : '')?>>
    ...
    <div class="card-product__content">
        <div class="card-product__about">
            ...
            <? endif; ?>
                <div class="card-product__about_name"><?=$productTitle?></div>
            <? if ($itemHasDetailUrl): ?>
            </a>
            <? endif; ?>
    ...
    <? else :
        ...
        if ($arParams['PRODUCT_DISPLAY_MODE'] === 'Y') { ?>
    <div class="__product-item-button-container">
        <? if ($showSubscribe) {
            $APPLICATION->IncludeComponent(
                'bitrix:catalog.product.subscribe',
                '',
        ...
        <button class="__btn __btn-link <?=$buttonSizeClass?>"
                id="<?=$itemIds['NOT_AVAILABLE_MESS']?>" href="javascript:void(0)" rel="nofollow"
            <?=($actualItem['CAN_BUY'] ? 'style="display: none;"' : '')?>>
            <?=$arParams['MESS_NOT_AVAILABLE']?>
        </button>
        <div id="<?=$itemIds['BASKET_ACTIONS']?>" <?=($actualItem['CAN_BUY'] ? '' : 'style="display: none;"')?>>
            <button class="__btn __btn-primary <?=$buttonSizeClass?>" id="<?=$itemIds['BUY_LINK']?>"
                    href="javascript:void(0)" rel="nofollow">
                <?=($arParams['ADD_TO_BASKET_ACTION'] === 'BUY' ? $arParams['MESS_BTN_BUY'] : $arParams['MESS_BTN_ADD_TO_BASKET'])?>
            </button>
        </div>
    ...
    <div class="__product-item-button-container">
        <button class="__btn __btn-primary __<?=$buttonSizeClass?>" href="<?=$item['DETAIL_PAGE_URL']?>">
            <?=$arParams['MESS_BTN_DETAIL']?>
        </button>
    </div>
